    ...
    if (fp)
    {
        ...
        for (row = 0; row < MIN(MAX_HEIGHT - 1, *height); row++)
        {
            for (col = 0; col < MIN(MAX_WIDTH - 1, *width); col++)
            {
                ...
                average += imageArray[row][col];
            }
        }
        fclose(fp);
    }

    *tValue = (double)average / (double)(*height * *width);

    return;
}

void brightness(int offset, int imageArray[][MAX_WIDTH], int *height, int *width)
{
    int row = 0;
    int col = 0;
    for (row = 0; row < MIN(MAX_HEIGHT - 1, *height); ++row)
